...
class MyBot {
  ...
  constructor(conversationState, userState) {
    ...
    this.dialogs.add(
      new NumberPrompt(AGE_PROMPT, async prompt => {
        if (prompt.recognized.succeeded) {
          if (prompt.recognized.value <= 0) {
            await prompt.context.sendActivity(
              `Your age can't be less than zero.`
            );
            return false;
          } else {
            return true;
    ...
    this.dialogs.add(
      new WaterfallDialog(HELLO_USER, [this.displayProfile.bind(this)])
    );
  }
  ...
  async confirmAgePrompt(step) {
    ...
    await step.prompt(CONFIRM_PROMPT, "Do you want to give your age?", [
      "yes",
      "no"
    ]);
  }
  ...
  async promptForAge(step) {
    if (step.result && step.result.value === "yes") {
      return await step.prompt(AGE_PROMPT, `What is your age?`, {
        retryPrompt:
          "Sorry, please specify your age as a positive number or say cancel."
      });
    } else {
      ...
      if (turnContext.activity.membersAdded.length !== 0) {
        ...
        for (var idx in turnContext.activity.membersAdded) {
          ...
          if (
            ...
            const description = ["Hello, I am a bot under development"];
